...
in his student work, written under supervision of Karl Menger; published in 1936. 
...
More generally, $\MM^n(\kappa)$ will denote \emph{model $n$-space} of curvature $\kappa$;
that is,
\begin{itemize}
...
\begin{thm}{Exercise}
Give an example of a quadruple $x_1,x_2,x_3,x_4$ in a metric space
such that there is no quadruple 
$\tilde x_1,\tilde x_2,\tilde x_3,\tilde x_4\in\EE^3$ with
\[\dist{\tilde x_i}{\tilde x_j}{\EE^3}=\dist{x_i}{x_j}{\spc{X}}\]
for all $i$ and $j$.
\end{thm}

Wald noticed that a \textit{typical} quadruple $x_1,x_2,x_3,x_4$
...
Moreover, the same hold for \textit{any} quadruple if one use $[-\infty,\infty)$ instead of $\RR$ 
and say $\kappa_{\min}(x_1,x_2,x_3,x_4)=-\infty$ if there is \textit{almost} model quadruple in
$\MM^3(\kappa)$ for $\kappa\to -\infty$;
that is, for any $\eps>0$ there is a quadruple
$\tilde x_1,\tilde x_2,\tilde x_3,\tilde x_4\in\MM^3(-\tfrac1\eps)$
such that 
\[\dist{\tilde x_i}{\tilde x_j}{\MM^3(-\frac1\eps)}\lege\dist{x_i}{x_j}{\spc{X}}\pm\eps\]
for all $i$ and $j$.
...
if any quadruple $x_1,x_2,x_3,x_4\in \spc{X}$ we have 
...
\section{Geodesics and geodesic spaces}
...
holds, then we say that the quadruple meets \index{$\CBB(0)$ comparison}\emph{$\CBB(0)$ comparison}.

\begin{thm}{Definition}\label{def:CBB}
A metric space $\spc{X}$ has \index{$\CBB$}\emph{nonnegative curvature} in the sense of Alexandrov (briefly, $\spc{X}\in\CBB(0)$ if $\CBB(0)$ comparison
...
However, interesting things happening only for the so-called \textit{geodesic} or at least \textit{length spaces}.

\section{Length and length spaces}
...
The main part of the following theorem is due to Alexadr Alexandrov~\cite{alexandrov-1948}.
...
Convex body above is a compact convex subset that does not lie in a line, but might degenerate to a plane figure, say $F$.
