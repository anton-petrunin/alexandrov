...
so, if you see $\CAT(\kappa)$, then it is safe assuming that $\kappa$ is $0$ or~$1$.
...
Recall that a \index{triangle}\emph{triangle} $\trig xyz$ in a space $\spc{U}$ 
...
The same way, the natural map can be defined for the spherical model triangle $\modtrig{}({x}{y}{z})_{\SSS^2}$.
...
Suppose $\gamma_1,\gamma_2\:[0,1]\to \spc{U}$ be two geodesic paths in a complete geodesic $\CAT(0)$ space $\spc{U}$.
Show that
...
Let $\spc{W}$ be a 
proper geodesic space which has two closed 
