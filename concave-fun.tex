...
\eqlbl{eq:gamma-v}\]
The quadruple $\gamma_0(\eps)$, $\gamma_1(\eps)$, $\gamma_2(\eps)$, $\gamma_3(\eps)$ satisfied $\CBB(\kappa)$ comparison;
after rescaling the distances by $\tfrac1\eps$, it becomes the $\CBB(\eps^2\cdot\kappa)$ comparison.
Passing to the limit as $\eps\to 0$ and applying \ref{eq:gamma-v}, we get the $\CBB(0)$ comparison for $v_0$, $v_1$, $v_2$, $v_3$.
\qeds

\begin{thm}{Exercise}
Show that tangent spaces of $\CAT(\kappa)$ space are $\CAT(0)$.
\end{thm}
