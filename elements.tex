...
If we write $\trig p q r$, it means that we have made a choice of such a triangle.
...
If, in addition, it is homeomorphic to a surface (without boundary), then it is called \emph{polyhedral surface}.
...
Given a hinge $\hinge pxy$ in a polyhedral surface $P$, denote by $\mangle\hinge pxy$ the minimal of angle measures that the hinge cuts from $P$ at $p$.
...
We may assume that $p$ is a regular point; otherwise move it slightly and apply approximation.
...
it is usually applied to geodesic spaces (or, at least, length spaces that will be defined latter).
...
\begin{thm}{Theorem}
A metric space $\spc{X}$ is isometric to the surface of convex body in the Euclidean space if and only if $\spc{X}$ is a geodesic $\CBB(0)$ space that is homeomorphic to $\SSS^2$.
...
To do this we will need to introduce convergence of subsets in Euclidean space (Hausdorff cnvergence) and convergence of metric spaces (Gromov--Hausdorff convergence) that will be done in the next lecture.
